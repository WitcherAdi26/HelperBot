...
//       try {
        ...
        //     }
        // });
        // const botMessage = { sender: 'bot', text: response.data.response.result };
...
//     const uploadingStatus = {};
    ...
    //     uploadingStatus[files[i].name] = 'uploading';
...
//       setMessages((prevMessages) => [
//         ...prevMessages.filter((msg) => !msg.text.startsWith('Uploading')),
//         ...uploadedMessages,
//       ]);
...
//       setUploadingFiles((prevStatus) => {
//         const newStatus = { ...prevStatus };
//         Object.keys(newStatus).forEach((fileName) => {
//           newStatus[fileName] = 'retry';
//         });
//         return newStatus;
//       });
//     }
...
//             {message.type === 'file' ? (
//               <div className={`pdf-message ${uploadingFiles[message.fileName] === 'retry' ? 'retry' : ''} ${uploadingFiles[message.fileName] === 'uploading' ? 'glowing' : ''}`}>
//                 <AiFillFilePdf className="pdf-icon" />
//                 <span>{message.fileName}</span>
//                 {uploadingFiles[message.fileName] === 'retry' && <button onClick={handleFileUpload}>Retry</button>}
//               </div>
//             ) : (
    ...
    <div className="chat-container">
      <div className="chat-messages">
        {messages.map((message, index) => (
          <div key={index} className={`chat-message ${message.sender}`}>
            {message.type === 'file' ? (
              // ${uploadingFiles[message.fileName]?.status === 'retry' ? 'retry' : ''}${uploadingFiles[message.fileName]?.status === 'uploading' ? 'glowing' : ''}
